...
export default function AdminStudentView() {
  ...
  const updatingGradeMark = (id) => {
    ...
    axios
      .post("https://www.globalschoolofyoga.com/grade/api/adminViewStudGrade.php", dataToSend)
      .then((res) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (validate()) {
      ...
      axios
        .post(
          "https://www.globalschoolofyoga.com/grade/api/adminViewStudGrade.php",
          dataToSend
        )
        .then((response) => {
          if (response.data && response.data.status === 1) {
            ...
            // Fetch grades immediately after successful submission
            fetchGrades();
          } else {
  ...
  const fetchGrades = () => {
    ...
    axios
      .post("https://www.globalschoolofyoga.com/grade/api/admin/adminViewStudGrade.php", metaGrade)
      .then((response) => {
        if (response.data && response.data.status === 1) {
          const gradeStatusData = response.data.grades || [];
          const updatedGrades = gradeStatusData.map((item) => ({
            ...item,
